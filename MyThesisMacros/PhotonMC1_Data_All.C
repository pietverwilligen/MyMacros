...
void PhotonMC1_Data_All() {
  ...
  std::string namearray[] = {"MC1_PhotonEt","MC1_PhotonEta", "MC1_PhotonPhi", "MC1_PhotonHE", "MC1_PhotonTrkIso", "MC1_PhotonEcalIso", "MC1_PhotonHcalIso", "MC1_PhotonPS", "MC1_PhotonSIEIE", "MC1_PhotonSIPIP", "MC1_PhotonPoverE"};
 ...
 for (int ii=0; ii<entries; ++ii) {
    ...
    int nDx=D->GetNbinsX(); for(int j=1; j<=nDx; ++j) {Dtotal += D->GetBinContent(j);}
    int nAx=A->GetNbinsX(); for(int j=1; j<=nAx; ++j) {Atotal += A->GetBinContent(j);}
    std::cout<<"Entries in Data histo = "<<Dtotal<<" | Entries in All MC histo = "<<Atotal<<" | A/D = "<<Dtotal/Atotal<<std::endl;

    // All MC + DATA
    TCanvas * c1 = new TCanvas("c","canvas",800,600);
    c1->cd();
    c1->SetLogy(1);
    A->GetYaxis()->SetRangeUser(3e-4,1e6);
    ...
    TLegend *l1 = new TLegend(0.60,0.65,0.85,0.85,NULL,"brNDC");
    l1->SetLineColor(0);
    l1->SetLineStyle(0);
    l1->SetLineWidth(0);
