...
void PhotonTurnOn() {
 ...
 for (int ii=0; ii<entries; ++ii) {
   ...
   P_num->SetLineColor(1);
   P_num->SetLineStyle(1);
   P_num->SetLineWidth(2);
   P_den->SetLineColor(2);
   P_den->SetLineStyle(2);
   P_den->SetLineWidth(2);
    ...
    if(!rebin) {
      ...
      c1->cd();
      c1->SetLogy(1);
      P_den->GetYaxis()->SetRangeUser(3e-4,1e4);      
      ...
      P_den->GetYaxis()->SetTitle(yaxis.c_str());
      P_den->GetYaxis()->SetLabelSize(0.025);
      // A->GetYaxis()->SetTicks("+");
      // A->GetXaxis()->SetTitle(xaxis.c_str())
      P_den->GetXaxis()->SetLabelSize(0.0000025);
      P_den->GetXaxis()->SetTicks("+-");
      P_den->Draw("HIST");
      P_num->Draw("HISTsame");
      // D->Draw("sameE");
      // Z->Draw("sameH");
      ...
      l1->SetBorderSize(0);
      l1->AddEntry(P_den, "Photon PT Before MHT Cut","l");
      l1->AddEntry(P_num, "Photon PT After MHT Cut","l");
      l1->Draw();
      cmsPrelim(lumi,-1);
      c1->SetLogy(1);
      double r = 0.4;
      ...
      ratioPad->SetGridy();
      TH1* ratio = 0;
      ratio = (TH1F*)P_num->Clone();
      ratio->Divide(P_num, P_den, 1, 1, "b");
