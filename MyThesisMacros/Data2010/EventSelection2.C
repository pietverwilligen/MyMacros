...
void EventSelectionTable() {
  ...
  // Zinv cross section
  CrossSections.push_back(5760); //NNLO (4500@LO)
  // DATA
  double lumi = 36.14;
  std::vector<double> Scaling;
  for(int i=0; i<CrossSections.size(); ++i) {Scaling.push_back(CrossSections[i]*lumi/Events[i]);}
  for(int i=0; i<CrossSections.size(); ++i) {
    std::cout<<std::setw(50)<<RootFileNames[i]<<" : x-section = "<<std::setw(9)<<CrossSections[i]<<" | lumi = "<<lumi<<" | events = "<<std::setw(9)<<Events[i];
    std::cout<<" | scaling = "<<std::setw(12)<<Scaling[i]<<std::endl; 
  }
  ...
  for(int i=0; i<5; ++i) {
    Sample_EC.push_back( (TH1F*) EventCounters[0]->Clone("Sample_EC")); int n1 = Sample_EC[i]->GetNbinsX();  for (int j=1; j<=n1; ++j) {Sample_EC[i]->SetBinContent(j,0.0);}
    Sample_RA2.push_back((TH1F*) RA2Selection[0]->Clone("Sample_RA2")); int n2 = Sample_RA2[i]->GetNbinsX(); for (int j=1; j<=n2; ++j) {Sample_RA2[i]->SetBinContent(j,0.0);}
  }
  ...
  std::cout<<std::setw(26)<<"TABLE HEADER: "<<std::setw(26)<<" DATA "<<std::setw(22)<<"PHOTON "<<std::setw(26)<<" QCD "<<std::setw(26)<<" EWK "<<std::setw(26)<<" Zinv "<<std::endl;
  std::cout<<"--------------------------------------------------------------------------------------------------------------------------------------------------------------"<<std::endl;
  ...
  for(int i=0; i<7; ++i) {
    Origin_MC.push_back((TH1F*) Book_All[0]->Clone("Origin_MC"));        int n1 = Origin_MC[i]->GetNbinsX();  for (int j=1; j<=n1; ++j) {Origin_MC[i]->SetBinContent(j,0.0);}
    Origin_ZB.push_back((TH1F*) RootTFiles[10]->Get("RA2SEL/0_PT_All")); int n2 = Origin_ZB[i]->GetNbinsX();  for (int j=1; j<=n2; ++j) {Origin_ZB[i]->SetBinContent(j,0.0); Origin_ZB[i]->Sumw2();}
    Origin_DA.push_back((TH1F*) Book_All[0]->Clone("Origin_DA"));        int n3 = Origin_DA[i]->GetNbinsX();  for (int j=1; j<=n3; ++j) {Origin_DA[i]->SetBinContent(j,0.0);}
    for(int j=0; j<10; ++j) {
      Origin_MC[i]->Add(Book[i][j]);
    }
    // For Data the ID bins are not filled, but we can take the amount of entries
    Origin_DA[i]->Add(Book[i][11]);
  }
  // For Zboson ID bins and Photon bins are not filled, we take the amount of entries of another histogram
  Origin_ZB[0]->Add((TH1F*) RootTFiles[10]->Get("RA2SEL/0_PT_All")); Origin_ZB[0]->Scale(Scaling[10]);
  Origin_ZB[1]->Add((TH1F*) RootTFiles[10]->Get("RA2SEL/1_PT_AJC")); Origin_ZB[1]->Scale(Scaling[10]);
  Origin_ZB[2]->Add((TH1F*) RootTFiles[10]->Get("RA2SEL/2_PT_AHC")); Origin_ZB[2]->Scale(Scaling[10]);
  Origin_ZB[3]->Add((TH1F*) RootTFiles[10]->Get("RA2SEL/3_PT_AAC")); Origin_ZB[3]->Scale(Scaling[10]);
  Origin_ZB[4]->Add((TH1F*) RootTFiles[10]->Get("RA2SEL/4_PT_AMC")); Origin_ZB[4]->Scale(Scaling[10]);
  Origin_ZB[5]->Add((TH1F*) RootTFiles[10]->Get("RA2SEL/6_PT_HHS")); Origin_ZB[5]->Scale(Scaling[10]);
  Origin_ZB[6]->Add((TH1F*) RootTFiles[10]->Get("RA2SEL/7_PT_HMS")); Origin_ZB[6]->Scale(Scaling[10]);

  std::vector<double> DaEv, DaEr, ZbEv, ZbEr, PhEv, PhEr, FrEv, FrEr, DeEv, DeEr, ElEv, ElEr;
  for(int i=0; i<7; ++i)  {
    DaEv.push_back(Origin_DA[i]->GetBinContent(5)); DaEr.push_back(Origin_DA[i]->GetBinError(5));
    ZbEv.push_back(Origin_ZB[i]->GetEntries());     ZbEr.push_back(0); // N/A
    ...
    FrEv.push_back(Origin_MC[i]->GetBinContent(8)); FrEr.push_back(Origin_MC[i]->GetBinError(8));
    DeEv.push_back(Origin_MC[i]->GetBinContent(9)); DeEr.push_back(Origin_MC[i]->GetBinError(9));
  }

  std::cout<<std::setw(26)<<"TABLE HEADER: "<<std::setw(26)<<" DATA "<<std::setw(22)<<"DIRECT PHOTON "<<std::setw(26)<<" FRAGMENTATION PHOTON "<<std::setw(26)<<" SECONDARY PHOTON "<<std::setw(26)<<" MISTAGGED ELECTRON "<<std::endl;
  std::cout<<"--------------------------------------------------------------------------------------------------------------------------------------------------------------"<<std::endl;
  int k;
  k=0;
  std::cout<<std::setw(20)<<"Boson + 2 Jets: "<<"| "<<std::setw(12)<<DaEv[k]<<"pm"<<std::setw(12)<<DaEr[k]<<"| "<<std::setw(12)<<PhEv[k]<<"pm"<<std::setw(12)<<PhEr[k]<<"| "<<std::setw(12)<<FrEv[k];
  std::cout<<"pm"<<std::setw(12)<<FrEr[k]<<"| "<<std::setw(12)<<DeEv[k]<<"pm"<<std::setw(12)<<DeEr[k]<<std::setw(12)<<ElEv[k]<<"pm"<<std::setw(12)<<ElEr[k]<<std::setw(12)<<ZbEv[k]<<"pm"<<std::setw(12)<<ZbEr[k]<<std::endl;
  k=1;
  std::cout<<std::setw(20)<<"Boson + 3 Jets: "<<"| "<<std::setw(12)<<DaEv[k]<<"pm"<<std::setw(12)<<DaEr[k]<<"| "<<std::setw(12)<<PhEv[k]<<"pm"<<std::setw(12)<<PhEr[k]<<"| "<<std::setw(12)<<FrEv[k];
  std::cout<<"pm"<<std::setw(12)<<FrEr[k]<<"| "<<std::setw(12)<<DeEv[k]<<"pm"<<std::setw(12)<<DeEr[k]<<std::setw(12)<<ElEv[k]<<"pm"<<std::setw(12)<<ElEr[k]<<std::setw(12)<<ZbEv[k]<<"pm"<<std::setw(12)<<ZbEr[k]<<std::endl;
  k=2;
  std::cout<<std::setw(20)<<"HT > 300: "<<"| "<<std::setw(12)<<DaEv[k]<<"pm"<<std::setw(12)<<DaEr[k]<<"| "<<std::setw(12)<<PhEv[k]<<"pm"<<std::setw(12)<<PhEr[k]<<"| "<<std::setw(12)<<FrEv[k];
  std::cout<<"pm"<<std::setw(12)<<FrEr[k]<<"| "<<std::setw(12)<<DeEv[k]<<"pm"<<std::setw(12)<<DeEr[k]<<std::setw(12)<<ElEv[k]<<"pm"<<std::setw(12)<<ElEr[k]<<std::setw(12)<<ZbEv[k]<<"pm"<<std::setw(12)<<ZbEr[k]<<std::endl;
  k=3;
  std::cout<<std::setw(20)<<"QCD Angular Cuts: "<<"| "<<std::setw(12)<<DaEv[k]<<"pm"<<std::setw(12)<<DaEr[k]<<"| "<<std::setw(12)<<PhEv[k]<<"pm"<<std::setw(12)<<PhEr[k]<<"| "<<std::setw(12)<<FrEv[k];
  std::cout<<"pm"<<std::setw(12)<<FrEr[k]<<"| "<<std::setw(12)<<DeEv[k]<<"pm"<<std::setw(12)<<DeEr[k]<<std::setw(12)<<ElEv[k]<<"pm"<<std::setw(12)<<ElEr[k]<<std::setw(12)<<ZbEv[k]<<"pm"<<std::setw(12)<<ZbEr[k]<<std::endl;
  k=4;
  std::cout<<std::setw(20)<<"Baseline Selection: "<<"| "<<std::setw(12)<<DaEv[k]<<"pm"<<std::setw(12)<<DaEr[k]<<"| "<<std::setw(12)<<PhEv[k]<<"pm"<<std::setw(12)<<PhEr[k]<<"| "<<std::setw(12)<<FrEv[k];
  std::cout<<"pm"<<std::setw(12)<<FrEr[k]<<"| "<<std::setw(12)<<DeEv[k]<<"pm"<<std::setw(12)<<DeEr[k]<<std::setw(12)<<ElEv[k]<<"pm"<<std::setw(12)<<ElEr[k]<<std::setw(12)<<ZbEv[k]<<"pm"<<std::setw(12)<<ZbEr[k]<<std::endl;
  k=5;
  std::cout<<std::setw(20)<<"High HT Selection: "<<"| "<<std::setw(12)<<DaEv[k]<<"pm"<<std::setw(12)<<DaEr[k]<<"| "<<std::setw(12)<<PhEv[k]<<"pm"<<std::setw(12)<<PhEr[k]<<"| "<<std::setw(12)<<FrEv[k];
  std::cout<<"pm"<<std::setw(12)<<FrEr[k]<<"| "<<std::setw(12)<<DeEv[k]<<"pm"<<std::setw(12)<<DeEr[k]<<std::setw(12)<<ElEv[k]<<"pm"<<std::setw(12)<<ElEr[k]<<std::setw(12)<<ZbEv[k]<<"pm"<<std::setw(12)<<ZbEr[k]<<std::endl;
  k=6;
  std::cout<<std::setw(20)<<"High MHT Selection: "<<"| "<<std::setw(12)<<DaEv[k]<<"pm"<<std::setw(12)<<DaEr[k]<<"| "<<std::setw(12)<<PhEv[k]<<"pm"<<std::setw(12)<<PhEr[k]<<"| "<<std::setw(12)<<FrEv[k];
  std::cout<<"pm"<<std::setw(12)<<FrEr[k]<<"| "<<std::setw(12)<<DeEv[k]<<"pm"<<std::setw(12)<<DeEr[k]<<std::setw(12)<<ElEv[k]<<"pm"<<std::setw(12)<<ElEr[k]<<std::setw(12)<<ZbEv[k]<<"pm"<<std::setw(12)<<ZbEr[k]<<std::endl;
