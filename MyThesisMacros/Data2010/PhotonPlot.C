...
void PhotonPlot() {
  ...
  std::string dir = "RECO/RECO_RA2Sel_Egamma/";
  ...
  std::string namearray[] = {"PhotonEt_RA2Sel","PhotonEta_RA2Sel", "PhotonPhi_RA2Sel", "PhotonHE_RA2Sel", "PhotonTrkIso_RA2Sel", "PhotonEcalIso_RA2Sel", "PhotonHcalIso_RA2Sel", "PhotonPS_RA2Sel", "PhotonSIEIE_RA2Sel", "PhotonSIPIP_RA2Sel", "PhotonPoverE_RA2Sel"};
 ...
 for (int ii=0; ii<entries; ++ii) {
   ...
   Q1 = ((TH1F*) file_Q1->Get((dir+SelHisto).c_str())); Q1->Sumw2();
   Q2 = ((TH1F*) file_Q2->Get((dir+SelHisto).c_str())); Q2->Sumw2();
   Q3 = ((TH1F*) file_Q3->Get((dir+SelHisto).c_str())); Q3->Sumw2();
   Q4 = ((TH1F*) file_Q4->Get((dir+SelHisto).c_str())); Q4->Sumw2();
   E1 = ((TH1F*) file_E1->Get((dir+SelHisto).c_str())); E1->Sumw2();
   E2 = ((TH1F*) file_E2->Get((dir+SelHisto).c_str())); E2->Sumw2();
   E3 = ((TH1F*) file_E3->Get((dir+SelHisto).c_str())); E3->Sumw2();


   P1 = ((TH1F*) file_P1->Get((dir+SelHisto).c_str())); P1->Sumw2();
   P2 = ((TH1F*) file_P2->Get((dir+SelHisto).c_str())); P2->Sumw2();
   P3 = ((TH1F*) file_P3->Get((dir+SelHisto).c_str())); P3->Sumw2();

   D = ((TH1F*)  file_D->Get((dir+SelHisto).c_str())); D->Sumw2();
   // Final Histograms                                                                                                                                                                                          
   TH1F *Q, *P, *A, *E;
   ...
   E = (TH1F*) E1->Clone("EWK"); int n_E = E->GetNbinsX(); for (int i=1; i<=n_E; ++i) {E->SetBinContent(i,0.0); E->SetBinError(i,0.0);}
   ...
   if(!rebin) {
     ...
     l1->AddEntry(P, "GJets #gamma","f");
     l1->AddEntry(Q, "QCD #gamma","f");
     l1->AddEntry(E, "EWK #gamma","f");
     l1->Draw();
     cmsPrelim(lumi, 1);
     c1->SetLogy(1);
     ...
     double ptbinning [] = {0.0, 5.0, 10.0, 15.0, 20.0, 25.0, 30.0, 35.0, 40.0, 45.0, 50.0, 55.0, 60.0, 65.0, 70.0, 75.0, 80.0, 85.0, 90.0, 95.0, 100.0, 105.0, 110.0, 115.0, 120.0, 125.0, 130.0, 135.0, 140.0\
			    , 145.0, 150.0, 165.0, 180.0, 195.0, 210.0, 225.0, 240.0, 255.0, 270.0, 285.0, 300.0, 350.0, 400.0, 450.0, 500.0};
     PH = new TH1F("PH", "PH", 44, ptbinning); PH->Sumw2();
     QD = new TH1F("QD", "QD", 44, ptbinning); QD->Sumw2();
     EW = new TH1F("EW", "EW", 44, ptbinning); EW->Sumw2();
     DA = new TH1F("DA", "DA", 44, ptbinning); DA->Sumw2();
     AA = new TH1F("AA", "AA", 44, ptbinning); AA->Sumw2();
	 ...
	 if (bin_lvalue >=  ptbinning[jj-1] && bin_rvalue <= ptbinning[jj]) {
	   ...
	   QD->AddBinContent(jj,Q->GetBinContent(iii));
	   ...
	   QD->SetBinError(jj,sqrt(pow(QD->GetBinError(jj),2)+pow(Q->GetBinError(iii),2)));
     ...
     for(int j=31; j<41; ++j) {
       ...
       QD->SetBinContent(j, QD->GetBinContent(j)/3); QD->SetBinError(j, QD->GetBinError(j)/3);
     ...
     l1->AddEntry(PH, "GJets #gamma","f");
     l1->AddEntry(QD, "QCD #gamma","f");
     l1->AddEntry(EW, "EWK #gamma","f");
     l1->Draw();
     cmsPrelim(lumi, 1);
     c1->SetLogy(1);
     c1->SetBottomMargin(0.2 + 0.8*c1->GetBottomMargin()-0.2*c1->GetTopMargin());
