...
void PhotonMC2_Prompt_All() {
  ...
  std::string namearray[] = {"MC2_PhotonEt","MC2_PhotonEta", "MC2_PhotonPhi", "MC2_PhotonHE", "MC2_PhotonTrkIso", "MC2_PhotonEcalIso", "MC2_PhotonHcalIso", "MC2_PhotonPS", "MC2_PhotonSIEIE", "MC2_PhotonSIPIP", "MC2_PhotonPoverE"};
 ...
 for (int ii=0; ii<entries; ++ii) {
    ...
    // Print Out
    double Ptotal = 0, Atotal = 0;
    int nPx=P->GetNbinsX(); for(int j=1; j<=nPx; ++j) {Ptotal += P->GetBinContent(j) + Q_fra->GetBinContent(j) + Q_dir->GetBinContent(j);}
    int nAx=A->GetNbinsX(); for(int j=1; j<=nAx; ++j) {Atotal += A->GetBinContent(j);}
    std::cout<<"Entries in Prompt histo = "<<Ptotal<<" | Entries in All MC histo = "<<Atotal<<" | P/A = "<<Ptotal/Atotal<<std::endl;
    double Prompttotal = 0;
    int nPrx=Prompt->GetNbinsX(); for(int j=1; j<=nPrx; ++j) {Prompttotal += Prompt->GetBinContent(j);}
    std::cout<<"Entries in Prompt histo = "<<Prompttotal<<" | Entries in All MC histo = "<<Atotal<<" | Prompt/All = "<<Prompttotal/Atotal<<std::endl;
    ...
    ratio->GetYaxis()->SetTitle("Prompt #gamma / All #gamma");
    if(r < 0.3) {
