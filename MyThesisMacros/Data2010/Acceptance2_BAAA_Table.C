...
void Acceptance2_BAAA_AllPlots() {

  std::string NumHisto [] = {"Boson_0J_AA/Pt", "Boson_1J_AA/Pt", "Boson_2J_AA/Pt", "Boson_3J_AA/Pt", "Boson_0J_AA/Eta", "Boson_1J_AA/Eta", "Boson_2J_AA/Eta", "Boson_3J_AA/Eta"};
  std::string DenomHisto [] = {"Boson_0J_BA/Pt", "Boson_1J_BA/Pt", "Boson_2J_BA/Pt", "Boson_3J_BA/Pt", "Boson_0J_BA/Eta", "Boson_1J_BA/Eta", "Boson_2J_BA/Eta", "Boson_3J_BA/Eta"};
  std::string NameHisto[] = {"Boson_0J_BAAA_Pt", "Boson_1J_BAAA_Pt", "Boson_2J_BAAA_Pt", "Boson_3J_BAAA_Pt", "Boson_0J_BAAA_Eta", "Boson_1J_BAAA_Eta", "Boson_2J_BAAA_Eta", "Boson_3J_BAAA_Eta"};

  std::vector<std::string> NumHistos; for(int i=0;i<8; ++i) { NumHistos.push_back(NumHisto[i]); }
  std::vector<std::string> DenomHistos; for(int i=0;i<8; ++i) { DenomHistos.push_back(DenomHisto[i]); }
  std::vector<std::string> Names;  for(int i=0;i<8; ++i) {  Names.push_back(NameHisto[i]); }
  ...
  for (int ii=0; ii<8; ++ii) {
	...
	PH_num->SetBinError(j,   PH_num->GetBinError(j)/10);   PH_denom->SetBinError(j, PH_denom->GetBinError(j)/10);
      ...
      std::cout<<"Entries before Acceptance = "<<DenomTotal<<" | Entries after Acceptance = "<<NumTotal<<" | Acceptance efficiency = "<<NumTotal/DenomTotal<<std::endl;
    }
    ...
    if(ii==0 || ii==4) PH_num->GetYaxis()->SetRangeUser(3e-3,3e7);
    if(ii==1 || ii==5) PH_num->GetYaxis()->SetRangeUser(3e-3,3e6);
    if(ii==2 || ii==6) PH_num->GetYaxis()->SetRangeUser(3e-3,3e5);
    if(ii==3 || ii==7) PH_num->GetYaxis()->SetRangeUser(3e-3,3e4);
    ...
    l1->SetBorderSize(0);
    l1->AddEntry(PH_num,   "Prompt #gamma Before Acceptance","l");
    l1->AddEntry(PH_denom, "Prompt #gamma After Acceptance","l");
    l1->Draw();
    cmsPrelim(lumi, 0);
    PhotonPt->SetLogy(1);
  ...
  std::cout<<"Anything done, so why do you crash?"<<std::endl;
}
