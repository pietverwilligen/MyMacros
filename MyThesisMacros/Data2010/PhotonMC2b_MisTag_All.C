...
void PhotonMC2b_MisTag_All() {
  ...
  std::string namearray[] = {"MC2b_PhotonEt","MC2b_PhotonEta", "MC2b_PhotonPhi", "MC2b_PhotonHE", "MC2b_PhotonTrkIso", "MC2b_PhotonEcalIso", "MC2b_PhotonHcalIso", "MC2b_PhotonPS", "MC2b_PhotonSIEIE", "MC2b_PhotonSIPIP", "MC2b_PhotonPoverE"};
 ...
 for (int ii=0; ii<entries; ++ii) {
    ...
    // Print Out
    double Etotal = 0, Atotal = 0;
    int nEx=E->GetNbinsX(); for(int j=1; j<=nEx; ++j) {Etotal += E->GetBinContent(j);}
    int nAx=A->GetNbinsX(); for(int j=1; j<=nAx; ++j) {Atotal += A->GetBinContent(j);}
    std::cout<<"Entries in MisTag histo = "<<Etotal<<" | Entries in All MC histo = "<<Atotal<<" | MT/A = "<<Etotal/Atotal<<std::endl;
