...
void PhotonPreSelectionPlotOverview() {

  std::string SelHisto = "PB_Et_B";
  std::string xaxis = "E_{T} [GeV]";
  std::string yaxis = "#gamma candidates / 5 GeV [-]";
  ...
  */
  ...
  Q->SetFillColor(kViolet);    as->Add(Q);
   ...
   D->Draw("E1SAME");
   // Coordinates for TLegend: x1, y1, x2, y2
   TLegend *l4 = new TLegend(0.70,0.65,0.85,0.80,NULL,"brNDC");
   l4->SetLineColor(0);
   l4->SetLineStyle(0);
   l4->SetLineWidth(0);
