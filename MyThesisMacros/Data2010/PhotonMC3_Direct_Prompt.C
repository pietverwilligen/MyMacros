...
void PhotonMC3_Direct_Prompt() {
  ...
  std::string namearray[] = {"MC3_PhotonEt","MC3_PhotonEta", "MC3_PhotonPhi", "MC3_PhotonHE", "MC3_PhotonTrkIso", "MC3_PhotonEcalIso", "MC3_PhotonHcalIso", "MC3_PhotonPS", "MC3_PhotonSIEIE", "MC3_PhotonSIPIP", "MC3_PhotonPoverE"};
 ...
 for (int ii=0; ii<entries; ++ii) {
    ...
    // Print Out
    double Prompttotal = 0, Ptotal = 0;
    int nPrx=Prompt->GetNbinsX(); for(int j=1; j<=nPrx; ++j) {Prompttotal += Prompt->GetBinContent(j);}
    int nPx=P->GetNbinsX(); for(int j=1; j<=nPx; ++j) {Ptotal += P->GetBinContent(j);}
    std::cout<<"Entries in Direct histo = "<<Ptotal<<" | Entries in Prompt histo = "<<Prompttotal<<" | Direct/Prompt = "<<Ptotal/Prompttotal<<std::endl;
    ...
    ratio->GetYaxis()->SetTitle("Direct #gamma / Prompt #gamma");
    if(r < 0.3) {
